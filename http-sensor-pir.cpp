...
#define PIR_FOLDER "/sensor-pir"

// Función para guardar imagen en SD
bool save_image_to_sd(const uint8_t* buf, size_t len) {
    // Crear carpeta si no existe
    if (!SD_MMC.exists(PIR_FOLDER)) {
        SD_MMC.mkdir(PIR_FOLDER);
    }
    // Generar nombre de archivo único
    String path = String(PIR_FOLDER) + "/pir_" + String(millis()) + ".jpg";
    File file = SD_MMC.open(path.c_str(), FILE_WRITE);
    if (!file) {
        Serial.println("No se pudo abrir el archivo en SD");
        return false;
    }
    file.write(buf, len);
    file.close();
    Serial.println("Imagen guardada en: " + path);
    return true;
}
        ...
        if (matched_id >= 0) {
            ...
            switch (matched_id) {
                case 0:
                    ...
                    rgb_printf(image_matrix, FACE_COLOR_CYAN, "!Bienvenido Luis!");
                    break;
                case 1:
                   
                    rgb_printf(image_matrix, FACE_COLOR_CYAN, "!Bienvenido Antonio!");
                    break;
                case 2:
                    
                    rgb_printf(image_matrix, FACE_COLOR_CYAN, "!Bienvenido Juan!");
                    break;
                case 3:
                    
                    rgb_printf(image_matrix, FACE_COLOR_CYAN, "!Bienvenido Pamela!");
                    break;
            }
        } else {
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&stream_httpd, &config) == ESP_OK) {
        ...
        // Loop para guardar imagen cuando PIR detecta movimiento
            ...
            [] (void*) {
                ...
                while (true) {
                    ...
                    if (pirState == HIGH && lastState == LOW) {
                        ...
                        delay(1000); // Evita múltiples capturas por un solo movimiento
                    }
                    lastState = pirState;
                    delay(100);
                }
            },
            "pir_task", 4096, NULL, 1, NULL, 1
        );
}
